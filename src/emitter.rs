...
fn check_aot_result(result: c_int) -> Result<(), Error> {
    if result == 0 {
        ...
        Err(Error::External(AotError::Dynasm(result).to_string()))
    }
}
    ...
    pub fn next(&mut self) -> Result<usize, Error> {
        if self.next > MAXIMAL_TEMP_REGISTER {
            return Err(Error::External(
                AotError::LimitReachedMaximumTempRegisters.to_string(),
            ));
        }
        let value = self.next;
        self.next += 1;
...
impl Emitter {
    pub fn new(labels: usize, version: u32) -> Result<Emitter, Error> {
        ...
        if aot.is_null() {
            Err(Error::External(AotError::Dynasm(-1).to_string()))
        } else {
            let emitter = Emitter {
                aot,
    ...
    pub fn link(&mut self) -> Result<usize, Error> {
        ...
        if result != 0 {
            return Err(Error::External(AotError::Dynasm(result).to_string()));
        }
        Ok(buffer_size)
    }
