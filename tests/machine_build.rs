...
use ckb_vm_aot::{AotCode, AotCompilingMachine, AotMachine};
...
#[allow(dead_code)]
pub fn aot_v1_imcb<'a>(path: &str, code: &'a AotCode) -> AotMachine<'a> {
    let buffer: Bytes = std::fs::read(path).unwrap().into();
...
#[allow(dead_code)]
pub fn aot_v1_mop<'a>(path: &str, args: Vec<Bytes>, code: &'a AotCode) -> AotMachine<'a> {
    let buffer: Bytes = std::fs::read(path).unwrap().into();
