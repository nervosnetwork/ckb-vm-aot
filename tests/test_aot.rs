use ckb_vm::{
    machine::{asm::AsmCoreMachine, CoreMachine, VERSION0, VERSION1},
    memory::Memory,
...
pub fn test_aot_chaos_seed() {
    ...
    let mut machine1 = AotMachine::new(core1, Some(&code1));
    machine1
    ...
    let mut machine2 = AotMachine::new(core2, Some(&code2));
    machine2
        .load_program(&buffer, &vec!["read_memory".into()])
        .unwrap();
